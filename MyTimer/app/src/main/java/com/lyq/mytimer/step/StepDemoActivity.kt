...
import kotlinx.android.synthetic.main.activity_step_demo.*
import java.text.DecimalFormat
import java.util.*
import kotlin.collections.ArrayList

class StepDemoActivity : BaseActivity(), Handler.Callback {
    ...
    private fun getLayoutId(): Int {
        return R.layout.activity_step_demo
    }
    ...
    private fun startStepService() {
        ...
        if (StepCountCheckUtil.isSupportStepCountSensor(this)) {
            getRecordList()
            tvIsSupport.visibility = View.GONE
            setDatas()
            setupService()
        } else {
            tvStepNum.text = "0"
            tvIsSupport!!.visibility = View.VISIBLE
        }
    }
...
//        isBind = bindService(intent, conn, Context.BIND_AUTO_CREATE)
    ...
    private fun setDatas() {
        ...
        if (stepEntity != null) {
            ...
            //获取全局的步数
            tvStepNum.text = steps.toString()
            //计算总公里数
            tvKm.text = steps?.let { countTotalKM(it) }
        } else {
            //获取全局的步数
            tvStepNum.text = "0"
            //计算总公里数
